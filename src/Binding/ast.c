...
u8 getCastInformation(astType from, astType to) {
	switch (from.kind) {
		...
		case arrayType:
			if (from.kind == to.kind) {
				if (from.arrayInfo->capacity == to.arrayInfo->capacity || to.arrayInfo->capacity == 0) return  getCastInformation(from.arrayInfo->ofType, to.arrayInfo->ofType);
			}
			return CAST_ILLEGAL;
...
int create_ast(char* filename, ast *tree, parser *p, bool parseOnly) {
	{
		p->filename = filename;
